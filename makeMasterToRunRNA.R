...
makeToRunTable <- function(pset.list) {
    ...
    setkey(pset.genexp.dt, geneid, PSet, cellid)
    ...
    check.row <- function(cur.drug, cur.tissue, cur.gene) {
        ...
        cur.sens.tbl <- sens.num.dt.filt[Drug == cur.drug][tissueid == cur.tissue]
        ...
        cur.gene.dataset <- colnames(genes.table)[genes.table[cur.gene, ]]
        ...
        cur.genexp.dt <- pset.genexp.dt[geneid == cur.gene, ][cellid %in% cur.tissue.cellines]
...
## Hours to run: 24.597
check.row2 <- function(cur.drug, cur.tissue, cur.gene) {
    cur.tissue.cellines <- tissueCell.dt[tissueid == cur.tissue, cellid]
    ...
    passing.datasets <- merge(cur.genexp.dt[, .(cellid, PSet)], cur.sens.tbl[, .(cellid, PSet)], on = c("PSet", "cellid"))[, .N, PSet][N >= 20, unique(PSet)]
    ...
    system.time(toCheckTable[1:5000, check.row2(Drug, Tissue, Gene), by = .(Drug, Tissue, Gene)])

    p3 <- profvis({
    my.x <- toCheckTable[1:10000, check.row2(Drug, Tissue, Gene), by = .(Drug, Tissue, Gene)]
    })
    p3
