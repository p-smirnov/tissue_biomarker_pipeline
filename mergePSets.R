...
mergePSets <- function(mDataPSet, sensDataPSet, commonCellsOnly=FALSE, ...){
  ...
  #acell <- setdiff(rownames(cellInfo(mDataPSet)), rownames(cellInfo(sensDataPSet)))
	...
	new.cell.curation <- c(union(colnames(sensDataPSet@curation$sample), colnames(mDataPSet@curation$sample)), "dataset")
	...
	cell.curation.df[rownames(mDataPSet@curation$sample), colnames(mDataPSet@curation$sample)] <- mDataPSet@curation$sample
	cell.curation.df[rownames(sensDataPSet@curation$sample), colnames(sensDataPSet@curation$sample)] <- sensDataPSet@curation$sample
	cell.curation.df[setdiff(rownames(sensDataPSet@curation$sample), rownames(mDataPSet@curation$sample)), "dataset"] <- name(sensDataPSet)
	cell.curation.df[setdiff(rownames(mDataPSet@curation$sample), rownames(sensDataPSet@curation$sample)), "dataset"] <- name(mDataPSet)
	cell.curation.df[intersect(rownames(mDataPSet@curation$sample), rownames(sensDataPSet@curation$sample)), "dataset"] <- paste0(name(sensDataPSet), "///", name(mDataPSet))
	mergePSet@curation$sample <- cell.curation.df
