...
# GDSC1 <- readRDS(GDSC1, "~/Data/TBPInputs/mut_variant/GDSC1.rds")
# readRDS(CCLE.CTRPv2, "~/Data/TBPInputs/mut_variant/CCLE.CTRPv2.rds")
# readRDS(gCSI, "~/Data/TBPInputs/mut_variant/gCSI.rds")


home <- Sys.getenv("HOME")
scratch <- Sys.getenv("SCRATCH")

myDataDir <- file.path(home, "Data", "TBPInputs", "mut_variant")
...
pset.list <- list(CTRPv2, GDSC1, gCSI)
...
cells_per_dataset <- sapply(pset.list, function(pset) {
    return(cellNames(pset)[cellInfo(pset)$tissueid %in% tissues_in_3])
})
...
pset.list.genexp <- lapply(pset.list, function(pset) summarizeMolecularProfiles(pset, mDataNames(pset), summary.stat="and"))
...
## Keep only those associations that have more than 3 actually mutated cell lines in at least 1 dataset

pset.genexp.dt <- merge(pset.genexp.dt, pset.genexp.dt[, sum(as.numeric(value)), .(PSet, tissueid, geneid)][V1 >= 3], by = c("PSet", "tissueid", "geneid"))
...
write.table(all.dt, file.path("~/Code/tissue_biomarker/MutationByVariantMasterToRunList.txt"), quote = FALSE, row.names = FALSE, sep = ",", col.names = FALSE)
